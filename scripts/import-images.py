...
for image in pass_list:
	cmdline = ['skopeo', '--insecure-policy', 'copy', '--dest-cert-dir=/etc/docker/certs.d/repo.home.nicknach.net', '--src-tls-verify=false', "docker://%s/%s:%s"%(args.source, image, args.tag), "%s/%s:%s"%(uri_string, image, args.tag)]
	if args.d: print('- '+' '.join(cmdline))
	try:
		check_call(cmdline, stdout=DEVNULL, stderr=STDOUT)
	except KeyboardInterrupt:
