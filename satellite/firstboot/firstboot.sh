...
cat <<EOF > post.sh
cd /root
cp /etc/origin/node/pods/apiserver.yaml /root/apiserver.yaml.prepatch
oc ex config patch /root/apiserver.yaml.prepatch -p '{"spec":{"containers":[{"name":"api","volumeMounts":[{"mountPath":"/etc/pki","name":"certs"}]}],"volumes":[{"hostPath":{"path":"/etc/pki","type":"Directory"},"name":"certs"}]}}' > /etc/origin/node/pods/apiserver.yaml 
